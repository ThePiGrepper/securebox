...
void rpiDrv_Setup(void){
  ...
  GPIO_InitStructure.GPIO_Pin = RPI_SCK_PIN | RPI_NSS_PIN;
  GPIO_InitStructure.GPIO_Speed =GPIO_Fast_Speed;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_DOWN;
  GPIO_Init(RPI_PORT, &GPIO_InitStructure);
  GPIO_InitStructure.GPIO_Pin = RPI_MISO_PIN | RPI_MOSI_PIN;
  GPIO_Init(RPI_PORT2, &GPIO_InitStructure);

  GPIO_PinAFConfig(RPI_PORT, RPI_NSS_PINSOURCE, RPI_MODULE_AF);
  GPIO_PinAFConfig(RPI_PORT, RPI_SCK_PINSOURCE, RPI_MODULE_AF);
  GPIO_PinAFConfig(RPI_PORT2, RPI_MISO_PINSOURCE, RPI_MODULE_AF);
  GPIO_PinAFConfig(RPI_PORT2, RPI_MOSI_PINSOURCE, RPI_MODULE_AF);
